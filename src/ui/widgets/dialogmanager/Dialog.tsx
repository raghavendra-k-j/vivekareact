...
export type DialogOverlayProps = { className?: string };
...
export function DialogOverlay(props: DialogOverlayProps) {
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    // Only consider presses that start on the overlay itself
    pressedOnSelf.current = e.target === e.currentTarget;
  };

  const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    // Close only if the press also ends on the overlay
    if (pressedOnSelf.current && e.target === e.currentTarget) {
      dialog.onClose?.();
    }
  ...
  return (
    <div
      className={clsx("dialog-overlay", props.className)}
...
export function DialogScaffold(
  { children, className, ...rest }: DialogScaffoldProps & React.HTMLAttributes<HTMLDivElement>
) {
  const dialog = useDialog();
  const pressedOnSelf = useRef(false);
  ...
  return (
    <div
      className={clsx("dialog-scaffold", className)}
      onPointerDown={handlePointerDown}
      onPointerUp={handlePointerUp}
      {...rest}
    >
...
export function DialogBottomSheetScaffold(
  { children, className, ...rest }: DialogBottomSheetScaffoldProps & React.HTMLAttributes<HTMLDivElement>
) {
  return (
