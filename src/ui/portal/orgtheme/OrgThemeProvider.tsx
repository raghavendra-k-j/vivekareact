...
import { ColorStop } from "~/domain/common/models/OrgTheme";
import { updateBrowserThemeColor } from "~/ui/utils/headUtils";
import { useAppStore } from "../layout/app/AppContext";

export default function OrgThemeProvider() {
    const appStore = useAppStore();
    const orgTheme = appStore.orgConfig.theme;

    const cp = (k: ColorStop) => orgTheme.primary.scale.get(k)!;
    ...
    useEffect(() => {
        updateBrowserThemeColor(orgTheme.bc.bg);
    }, [orgTheme]);

    return (
        <div
            style={themeVariables as React.CSSProperties}
            className="h-full flex-1 overflow-hidden flex flex-col min-h-0"
        >
            <Outlet />
        </div>
    );
